...
declare variable $ead as document-node()* := doc("C1567.EAD.xml");
...
(: Added replace function to remove "'"s from string because they were causing problems with the json. :)

($f/ancestor::ead:c[1]/@id || '*' || $f/name() || '*' || replace(normalize-space($f/string()), "'", "") || codepoints-to-string(10))
